...
/* * * * * * * Abstract Methods * * * * * * */
	...
	/**
	 * 블록의 색상을 구합니다.<br>
	 * 블록의 색상은 클래스명을 기준으로 생성됩니다.<br>
	 * 이 메소드를 상속받아서 원하는 색상으로 만들 수 있습니다.
	 */
	public Color getColor() {
		if(color == null) {
			try {
				...
				Random rand = new Random(Long.parseLong(r.toString().substring(24), 16));
	...
	public void restoreState(final Map<String, String> attrs) throws ParseException {
		...
		if(xStr != null) {
			try {
				...
				LoggerFactory.getLogger(BaseBlockModel.class).warn("Cannot resolve saved state:", e);
				setLocation(0, 0);
			}
		} else {
			setLocation(0, 0);
